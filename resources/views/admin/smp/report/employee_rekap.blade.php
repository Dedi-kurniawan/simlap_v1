...
<body class="A4 landscape">
    <section class="sheet padding-10mm">
        <div class="kop text-center">
            <h4 class="text-uppercase">REKAPITULASI LAPORAN BULANAN SEKOLAH <br>TENAGA KEPENDIDIKAN <br>BULAN {{ $data['bulan'] }}</h4>
        </div>
        ...
        <table class="tg" style="width: 100%">
            ...
            <tbody>
                ...
                @php
                    $laki_r += $t->employeeReportSmp->where('jenis_kelamin', 'L')->count();
                    $perempuan_r += $t->employeeReportSmp->where('jenis_kelamin', 'P')->count();
                    $pns_r += $t->employeeReportSmp->where('status_pegawai', 'PNS')->count();
                @endphp
                    <tr>
                        <td class="tg-qr5b text-center">{{ $loop->iteration }}</td>
                        <td class="tg-qr5b">{{ $t->nama_sekolah }}</td>
                        <td class="tg-qr5b text-center">{{ $laki = $t->employeeReportSmp->where('jenis_kelamin', 'L')->count() }}</td>
                        <td class="tg-qr5b text-center">{{ $perempuan = $t->employeeReportSmp->where('jenis_kelamin', 'P')->count() }}</td>
                        <td class="tg-qr5b text-center">{{ $pns = $t->employeeReportSmp->where('status_pegawai', 'PNS')->count() }}</td>
