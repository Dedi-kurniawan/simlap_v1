...
class DashboardController extends Controller
{
    ...
    {
        $bcrum = $this->bcrum('Dashboard', 'MAIN MENU', route('smp.dashboard'), 'Dashboard');
        $settings = Setting::distinct()->orderBy('tahun', 'asc')->get(['tahun']);
        $setting = $this->reportActive();
        $report = $this->report($this->schoolId(), $setting['bulan-save'], $setting['tahun']);
        return view('school.smp.dashboards.index', compact('bcrum', 'settings', 'report', 'setting'));
    }

    public function report($school,$bulan, $tahun)
    {
        $report  = Report::OfSchoolId($school)->OfBulan($bulan)->OfTahun($tahun)->first();
        if ($report) {
                return [
