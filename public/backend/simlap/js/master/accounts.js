...
function createData() {
    $(".modal-title").html("Formulir Tambah Akun");
    $("#submitData").val("add");
    $(".method-hidden").html("");
...
$(document).on("click", "#editData", function (e) {
    ...
    if (data) {
        var id = data.id;
        var url   = HOST_URL + '/admin/akun/' + id + '/edit';
        $.get(url, function (d) {
            if (d.status == 'success') {
                $('#nama_sekolah').val(d.edit.school_id).trigger('change');
                $('#role_id').val(d.edit.role_id).trigger('change');
                $('#name').val(d.edit.name);
                $('#email').val(d.edit.email);
                $("#id_edit").val(id);
                $(".modal-title").html("Formulir Ubah Data Akun");
                $("#submitData").val("edit");
                $("#formModal").modal("show");
...
$("#submitData").click(function (event) {
    ...
    if (action == "add") {
        var url = HOST_URL + '/admin/akun';
        var title = "TAMBAH DATA";
    } else if (action == "edit") {
        var id    = $("#id_edit").val();
        var url   = HOST_URL + '/admin/akun/' + id;
        var title = "UBAH DATA";
    }
...
function resetPassword(id) {
    ...
    var action = "delete";
    $.ajax({
        url: HOST_URL + '/admin/akun/reset/' + id,
        method: "POST",
        dataType: 'json',
        success: function (d) {
            finishLoading();
