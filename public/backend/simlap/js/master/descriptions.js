...
var table = $('#datatable').DataTable({
    ...
    ajax: {
        method: 'POST',
    	url: HOST_URL + '/dt/admin/master/fasilitas',
    },
    columns: [{
            data: 'DT_RowIndex',
            orderable: false,
...
function createData() {
    $(".modal-title").html("Formulir Tambah Data URAIAN");
    $("#submitData").val("add");
    $(".method-hidden").html("");
...
$(document).on("click", "#editData", function (e) {
    ...
    if (data) {
        var id = data.id;
        var url   = HOST_URL + '/admin/master/fasilitas/' + id + '/edit';
        $.get(url, function (d) {
            if (d.status == 'success') {
                ...
                $("#id_edit").val(id);
                $(".modal-title").html("Formulir Ubah Data URAIAN");
                $("#submitData").val("edit");
                $("#formModal").modal("show");
...
$("#submitData").click(function (event) {
    ...
    if (action == "add") {
        var url = HOST_URL + '/admin/master/fasilitas';
        var title = "TAMBAH DATA";
    } else if (action == "edit") {
        var id    = $("#id_edit").val();
        var url   = HOST_URL + '/admin/master/fasilitas/' + id;
        var title = "UBAH DATA";
    }
...
function deleteData(id) {
    ...
    var action = "delete";
    $.ajax({
        url: HOST_URL + '/admin/master/fasilitas/' + id,
        method: "DELETE",
        dataType: 'json',
        success: function (d) {
            console.log(d);
