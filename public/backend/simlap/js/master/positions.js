...
var table = $('#datatable').DataTable({
    ...
    ajax: {
        method: 'POST',
    	url: HOST_URL + '/dt/admin/master/jabatan',
    },
    columns: [{
            data: 'DT_RowIndex',
            orderable: false,
...
function createData() {
    $(".modal-title").html("Formulir Tambah Data JABATAN");
    $("#submitData").val("add");
    $(".method-hidden").html("");
...
$(document).on("click", "#editData", function (e) {
    ...
    if (data) {
        var id = data.id;
        var url   = HOST_URL + '/admin/master/jabatan/' + id + '/edit';
        $.get(url, function (d) {
            if (d.status == 'success') {
                ...
                $("#id_edit").val(id);
                $(".modal-title").html("Formulir Ubah Data JABATAN");
                $("#submitData").val("edit");
                $("#formModal").modal("show");
...
$("#submitData").click(function (event) {
    ...
    if (action == "add") {
        var url = HOST_URL + '/admin/master/jabatan';
        var title = "TAMBAH DATA";
    } else if (action == "edit") {
        var id    = $("#id_edit").val();
        var url   = HOST_URL + '/admin/master/jabatan/' + id;
        var title = "UBAH DATA";
    }
...
function deleteData(id) {
    ...
    var action = "delete";
    $.ajax({
        url: HOST_URL + '/admin/master/jabatan/' + id,
        method: "DELETE",
        dataType: 'json',
        success: function (d) {
            finishLoading();
