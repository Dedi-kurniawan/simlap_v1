...
var table = $('#datatable').DataTable({
    ...
    ajax: {
        method: 'POST',
    	url: HOST_URL + '/dt/admin/paud/pengaturan',
    },
    columns: [{
            data: 'DT_RowIndex',
            orderable: false,
...
$(document).on("click", "#editData", function (e) {
    ...
    if (data) {
        var id = data.id;
        var url   = HOST_URL + '/admin/paud/pengaturan/' + id + '/edit';
        $.get(url, function (d) {
            if (d.status == 'success') {
...
$("#submitData").click(function (event) {
    ...
    if (action == "add") {
        var url = HOST_URL + '/admin/paud/pengaturan';
        var title = "TAMBAH DATA";
    } else if (action == "edit") {
        var id    = $("#id_edit").val();
        var url   = HOST_URL + '/admin/paud/pengaturan/' + id;
        var title = "UBAH DATA";
    }
...
function deleteData(id) {
    ...
    var action = "delete";
    $.ajax({
        url: HOST_URL + '/admin/paud/pengaturan/' + id,
        method: "DELETE",
        dataType: 'json',
        success: function (d) {
            finishLoading();
...
function updateStatus(id, status) {
    ...
    var action = "edit";
    $.ajax({
        url: HOST_URL + '/admin/paud/pengaturan/status/'+id,
        method: 'POST',
        dataType: 'json',
        data: {
            status  : status,
