...
var table = $('#datatable').DataTable({
    ...
    ajax: {
        method: 'POST',
    	url: HOST_URL + '/dt/paud/tenaga-kependidikan',
    },
    columns: [{
        data: 'DT_RowIndex',
        orderable: false,
...
function createData() {
    $(".modal-title").html("Formulir Tambah Data Tenaga Kependidikan paud");
    $("#submitData").val("add");
...
$(document).on("click", "#editData", function (e) {
    ...
    if (data) {
        var id = data.id;
        var url   = HOST_URL + '/paud/tenaga-kependidikan/' + id + '/edit';
        $.get(url, function (d) {
            if (d.status == 'success') {
                ...
                $("#id_edit").val(id);
                $(".modal-title").html("Formulir Ubah Data Tenaga Kependidikan paud");
                $("#submitData").val("edit");
                $("#formModal").modal("show");
...
$("#submitData").click(function (event) {
    ...
    if (action == "add") {
        var url = HOST_URL + '/paud/tenaga-kependidikan';
        var title = "TAMBAH DATA";
    } else if (action == "edit") {
        var id    = $("#id_edit").val();
        var url   = HOST_URL + '/paud/tenaga-kependidikan/' + id;
        var title = "UBAH DATA";
    }
    ...
    $.ajax({
        ...
        error: function (json) {
            ...
            $.each(errors.errors, function (key, value) {
                if (key == 'status_pegawai') {
                    $(".error-status_pegawai").html("<label id='" + key + "-error' class='error ' for='" + key + "'>" + value + "</label>");
                }
...
function deleteData(id) {
    ...
    var action = "delete";
    $.ajax({
        url: HOST_URL + '/paud/tenaga-kependidikan/' + id,
        method: "DELETE",
        dataType: 'json',
        success: function (d) {
            finishLoading();
...
$("#generateReport").click(function (event) {
    ...
    $.ajax({
        url: HOST_URL + '/paud/report-tenaga-kependidikan',
        method: "POST",
        error: function (json) {
