...
var table = $('#datatable').DataTable({
    ...
    ajax: {
        method: 'POST',
    	url: HOST_URL + '/dt/paud/tenaga-pendidik',
    },
    columns: [{
            data: 'DT_RowIndex',
            orderable: false,
...
function createData() {
    $(".modal-title").html("Formulir Tambah Data Tenaga Pendidik PAUD");
    $("#submitData").val("add");
...
$(document).on("click", "#editData", function (e) {
    ...
    if (data) {
        var id = data.id;
        var url   = HOST_URL + '/paud/tenaga-pendidik/' + id + '/edit';
        $.get(url, function (d) {
            if (d.status == 'success') {
                $("#sertifikasi_tahun").val(d.edit.sertifikasi_tahun);
                $("#pelatihan").val(d.edit.pelatihan);
                $('#nama_lengkap').val(d.edit.nama); 
                $('#nip').val(d.edit.nip); 
                $('#tanggal_mulai_bekerja').val(d.edit.tanggal_mulai_bekerja); 
                $('#golongan').val(d.edit.golongan == null ? "" : d.edit.golongan).trigger('change');
                $('#tmt_gol_terakhir').val(d.edit.tmt_gol_terakhir); 
                $('#tmt_capeg').val(d.edit.tmt_sekolah); 
                $('#jabatan').val(d.edit.jabatan == null ? "" : d.edit.jabatan).trigger('change'); 
                $('#tmt_jabatan').val(d.edit.tmt_jabatan);
                $('#tmt_capeg').val(d.edit.tmt_capeg); 
                ...
                $('#tugas_mengajar').val(d.edit.tugas_mengajar == null ? "" : d.edit.tugas_mengajar).trigger('change');
                ...
                $("#id_edit").val(id);
                $(".modal-title").html("Formulir Ubah Data Tenaga Pendidik PAUD");
                $("#submitData").val("edit");
                $("#formModal").modal("show");
...
$("#submitData").click(function (event) {
    ...
    if (action == "add") {
        var url = HOST_URL + '/paud/tenaga-pendidik';
        var title = "TAMBAH DATA";
    } else if (action == "edit") {
        var id    = $("#id_edit").val();
        var url   = HOST_URL + '/paud/tenaga-pendidik/' + id;
        var title = "UBAH DATA";
    }
...
function deleteData(id) {
    ...
    var action = "delete";
    $.ajax({
        url: HOST_URL + '/paud/tenaga-pendidik/' + id,
        method: "DELETE",
        dataType: 'json',
        success: function (d) {
            finishLoading();
...
$("#generateReport").click(function (event) {
    ...
    $.ajax({
        url: HOST_URL + '/paud/report-tenaga-pendidik',
        method: "POST",
        error: function (json) {
