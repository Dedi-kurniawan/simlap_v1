...
var table = $('#datatable').DataTable({
    ...
    ajax: {
        method: 'POST',
    	url: HOST_URL + '/dt/paud/peserta-didik',
    },
    columns: [{
            data: 'DT_RowIndex',
            orderable: false,
...
function createData() {
    $(".modal-title").html("Formulir Tambah Data Peserta didik paud");
    $("#submitData").val("add");
    $(".method-hidden").html("");
...
$(document).on("click", "#editData", function (e) {
    ...
    if (data) {
        var id = data.id;
        var url   = HOST_URL + '/paud/peserta-didik/' + id + '/edit';
        $.get(url, function (d) {
            if (d.status == 'success') {
                $('#room_id').val(d.edit.room_id).trigger('change'); 
                $('#siswa_l').val(d.edit.siswa_l); 
                $('#siswa_p').val(d.edit.siswa_p);  
                $('#usia_2').val(d.edit.usia_2);  
                $('#usia_3').val(d.edit.usia_3);  
                $('#usia_4').val(d.edit.usia_4);  
                $('#usia_5').val(d.edit.usia_5);  
...
$("#submitData").click(function (event) {
    ...
    if (action == "add") {
        var url = HOST_URL + '/paud/peserta-didik';
        var title = "TAMBAH DATA";
    } else if (action == "edit") {
        var id    = $("#id_edit").val();
        var url   = HOST_URL + '/paud/peserta-didik/' + id;
        var title = "UBAH DATA";
    }
...
function deleteData(id) {
    ...
    var action = "delete";
    $.ajax({
        url: HOST_URL + '/paud/peserta-didik/' + id,
        method: "DELETE",
        dataType: 'json',
        success: function (d) {
            finishLoading();
...
$("#generateReport").click(function (event) {
    ...
    $.ajax({
        url: HOST_URL + '/paud/report-peserta-didik',
        method: "POST",
        error: function (d) {
            console.log(d);
