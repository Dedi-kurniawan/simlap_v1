...
var table = $('#datatable').DataTable({
    ...
    ajax: {
        method: 'POST',
    	url: HOST_URL + '/dt/smp/kebutuhan-guru',
    },
    columns: [{
            data: 'DT_RowIndex',
            orderable: false,
...
$(document).on("click", "#editData", function (e) {
    ...
    if (data) {
        var id = data.id;
        var url   = HOST_URL + '/smp/kebutuhan-guru/' + id + '/edit';
        $.get(url, function (d) {
            if (d.status == 'success') {
...
$("#submitData").click(function (event) {
    ...
    if (action == "add") {
        var url = HOST_URL + '/smp/kebutuhan-guru';
        var title = "TAMBAH DATA";
    } else if (action == "edit") {
        var id    = $("#id_edit").val();
        var url   = HOST_URL + '/smp/kebutuhan-guru/' + id;
        var title = "UBAH DATA";
    }
...
function deleteData(id) {
    ...
    var action = "delete";
    $.ajax({
        url: HOST_URL + '/smp/kebutuhan-guru/' + id,
        method: "DELETE",
        dataType: 'json',
        success: function (d) {
            finishLoading();
...
$("#generateReport").click(function (event) {
    ...
    $.ajax({
        url: HOST_URL + '/smp/report-kebutuhan-guru',
        method: "POST",
        error: function (d) {
            console.log(d);
