...
var table = $('#datatable').DataTable({
    ...
    ajax: {
        method: 'POST',
    	url: HOST_URL + '/dt/smp/tenaga-kependidikan',
    },
    columns: [{
            data: 'DT_RowIndex',
            orderable: false,
    ...
    buttons: [{
            ...
            messageTop: 'DATA TENAGA KEPENDIDIKAN',
...
function createData() {
    $(".modal-title").html("Formulir Tambah Data Tenaga Kependidikan");
    $("#submitData").val("add");
...
$(document).on("click", "#editData", function (e) {
    ...
    if (data) {
        var id = data.id;
        var url   = HOST_URL + '/smp/tenaga-kependidikan/' + id + '/edit';
        $.get(url, function (d) {
            if (d.status == 'success') {
                ...
                $('#major_id').val(d.edit.major_id).trigger('change');                
                $('#jjm').val(d.edit.jjm);
                $("#id_edit").val(id);
                $(".modal-title").html("Formulir Ubah Data Tenaga Kependidikan");
                $("#submitData").val("edit");
                $("#formModal").modal("show");
...
$("#submitData").click(function (event) {
    ...
    if (action == "add") {
        var url = HOST_URL + '/smp/tenaga-kependidikan';
        var title = "TAMBAH DATA";
    } else if (action == "edit") {
        var id    = $("#id_edit").val();
        var url   = HOST_URL + '/smp/tenaga-kependidikan/' + id;
        var title = "UBAH DATA";
    }
...
function deleteData(id) {
    ...
    var action = "delete";
    $.ajax({
        url: HOST_URL + '/smp/tenaga-kependidikan/' + id,
        method: "DELETE",
        dataType: 'json',
        success: function (d) {
            finishLoading();
...
$("#generateReport").click(function (event) {
    ...
    $.ajax({
        url: HOST_URL + '/smp/report-tenaga-kependidikan',
        method: "POST",
        error: function (json) {
