...
var table = $('#datatable').DataTable({
    ...
    ajax: {
        method: 'POST',
    	url: HOST_URL + '/dt/smp/tenaga-pendidik',
    },
    columns: [{
            data: 'DT_RowIndex',
            orderable: false,
...
function createData() {
    $(".modal-title").html("Formulir Tambah Data Tenaga Pendidik SMP");
    $("#submitData").val("add");
...
$(document).on("click", "#editData", function (e) {
    ...
    if (data) {
        var id = data.id;
        var url   = HOST_URL + '/smp/tenaga-pendidik/' + id + '/edit';
        $.get(url, function (d) {
            if (d.status == 'success') {
                ...
                $('#jjm').val(d.edit.jjm);
                $("#id_edit").val(id);
                $(".modal-title").html("Formulir Ubah Data Tenaga Pendidik SMP");
                $("#submitData").val("edit");
                $("#formModal").modal("show");
...
$("#submitData").click(function (event) {
    ...
    if (action == "add") {
        var url = HOST_URL + '/smp/tenaga-pendidik';
        var title = "TAMBAH DATA";
    } else if (action == "edit") {
        var id    = $("#id_edit").val();
        var url   = HOST_URL + '/smp/tenaga-pendidik/' + id;
        var title = "UBAH DATA";
    }
...
function deleteData(id) {
    ...
    var action = "delete";
    $.ajax({
        url: HOST_URL + '/smp/tenaga-pendidik/' + id,
        method: "DELETE",
        dataType: 'json',
        success: function (d) {
            finishLoading();
...
$("#generateReport").click(function (event) {
    ...
    $.ajax({
        url: HOST_URL + '/smp/report-tenaga-pendidik',
        method: "POST",
        error: function (d) {
