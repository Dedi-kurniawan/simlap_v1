...
var table = $('#datatable').DataTable({
    ...
    ajax: {
        method: 'POST',
    	url: HOST_URL + '/dt/sd/peserta-didik',
    },
    columns: [{
            data: 'DT_RowIndex',
            orderable: false,
...
function createData() {
    $(".modal-title").html("Formulir Tambah Data Peserta didik SD");
    $("#submitData").val("add");
    $(".method-hidden").html("");
...
$(document).on("click", "#editData", function (e) {
    ...
    if (data) {
        var id = data.id;
        var url   = HOST_URL + '/sd/peserta-didik/' + id + '/edit';
        $.get(url, function (d) {
            if (d.status == 'success') {
                $('#room_id').val(d.edit.room_id).trigger('change'); 
                $('#siswa_l').val(d.edit.siswa_l); 
                $('#siswa_p').val(d.edit.siswa_p);
                $('#usia_6').val(d.edit.usia_6);
                $('#usia_7').val(d.edit.usia_7);
                $('#usia_8').val(d.edit.usia_8);
                $('#usia_9').val(d.edit.usia_9);
                $('#usia_10').val(d.edit.usia_10);  
                ...
                $("#id_edit").val(id);
                $(".modal-title").html("Formulir Ubah Data Peserta Pendidik SD");
                $("#submitData").val("edit");
                $("#formModal").modal("show");
...
$("#submitData").click(function (event) {
    ...
    if (action == "add") {
        var url = HOST_URL + '/sd/peserta-didik';
        var title = "TAMBAH DATA";
    } else if (action == "edit") {
        var id    = $("#id_edit").val();
        var url   = HOST_URL + '/sd/peserta-didik/' + id;
        var title = "UBAH DATA";
    }
...
function deleteData(id) {
    ...
    var action = "delete";
    $.ajax({
        url: HOST_URL + '/sd/peserta-didik/' + id,
        method: "DELETE",
        dataType: 'json',
        success: function (d) {
            finishLoading();
...
$("#generateReport").click(function (event) {
    ...
    $.ajax({
        url: HOST_URL + '/sd/report-peserta-didik',
        method: "POST",
        error: function (d) {
            console.log(d);
